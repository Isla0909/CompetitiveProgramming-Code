...
struct SegmentTree
{
	...
	int find1(int u, int ql, int qr, i64 x)//查询区间第一个大于x的数的下标
	{
		//无交集返回-1
		...
		int c = find1(u << 1, ql, qr, x);
		if(c != -1) return c;
		return find1(u << 1 | 1, ql, qr, x);
	}

	int find2(int u, int ql, int qr, i64 x)//查询区间第一个小于x的数的下标
	{
		//无交集返回-1
		if(tree[u].l > qr || tree[u].r < ql) return -1;
		...
		int c = find2(u << 1, ql, qr, x);
		if(c != -1) return c;
		return find2(u << 1 | 1, ql, qr, x);
	}
};
...
signed main()
{
	...
	while(T --)
	{
		...
		SegmentTree seg(p); DSU dsu(n);
		for(int i = 1; i <= n; i ++)
		{
			int idx1 = seg.find1(1, i, n, p[i]), idx2 = seg.find2(1, 1, i, p[i]);
			if(idx1 != -1) dsu.merge(p[idx1], p[i]);
			else if(idx2 != -1) dsu.merge(p[idx2], p[i]);
		}
		bool f = (dsu.siz[dsu.find(1)] == n);
		if(f)
		{
			cout <<"Yes" <<endl;
			for(auto [u, v] : dsu.ans)
				cout <<u <<" " <<v <<endl;
		}
		else cout <<"No" <<endl;
	}
