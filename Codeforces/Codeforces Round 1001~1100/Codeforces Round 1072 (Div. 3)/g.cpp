...
struct SegmentTree
{
	...
	int find_bad(int u, int ql, int qr, int L, i64 &cur)
	{
		if(tree[u].r < ql || tree[u].l > qr) return -1;

		if(tree[u].l >= ql && tree[u].r <= qr)
		{
			i64 v = min(cur, tree[u].val2);
		...
		int res = find_bad(u << 1, ql, qr, L, cur);
		if(res != -1) return res;
		return find_bad(u << 1 | 1, ql, qr, L, cur);
	}
...
signed main()
{
	...
	while(T --)
	{
		...
		while(q --)
		{
			...
			{
				int pos, x; cin >>pos >>x;
				seg.rangeAdd(1, pos, pos, x - a[pos]);
				a[pos] = x;
			}
			else
			{
				int L, R; cin >>L >>R;
				i64 cur = inf;
				int pos = seg.find_bad(1, L, R, L, cur);
