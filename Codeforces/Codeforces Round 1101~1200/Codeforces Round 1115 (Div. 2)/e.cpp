...
signed main()
{
	...
	{
		i64 n; cin >>n;
		array<array<array<Z, 2>, 2>, 2> dp;
		dp[0][1][1] = 1;

		for(int i = 59; i >= 0; i --)
		{
			array<array<array<Z, 2>, 2>, 2> ndp;

			int d = n >> i & 1;

			for(int preb = 0; preb < 2; preb ++)
				for(int a = 0; a < 2; a ++)
					for(int c = 0; c < 2; c ++)
						for(int x = 0; x < 2; x ++)
							for(int y = 0; y < 2; y ++)
							{
								if((x & y) != preb) continue;

								int bi = x ^ y;
								if(i == 0 && bi) continue;

								if((a == 1 && x > d) || (c == 1 && y > d)) continue;

								int na = a & (x == d), nc = c & (y == d);
								ndp[bi][na][nc] += dp[preb][a][c];
							}	
			swap(dp, ndp);
		}
