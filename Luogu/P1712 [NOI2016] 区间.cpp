...
signed main()
{
	...
	for(int i = 0; i < n; i ++)
	{
		...
		query[i][2] = query[i][1] - query[i][0];
		v.push_back(query[i][0]), v.push_back(query[i][1]);
	}
	sort(v.begin(), v.end());
	...
	for(int i = 0; i < n; i ++)
	{
		auto &[l, r, len] = query[i];
		seg.rangeAdd(1, l, r, 1);

		while(j <= i && seg.max_query(1, 1, m) >= c)
		{
			ans = min(ans, query[i][2] - query[j][2]);
			auto &[prel, prer, prelen] = query[j];
			seg.rangeAdd(1, prel, prer, -1);
