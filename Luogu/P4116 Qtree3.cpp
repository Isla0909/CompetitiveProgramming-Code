...
struct SegmentTree
{
	...
	int find_first(int u, int ql, int qr)//查询区间第一个大于x的数的下标
	{
		//无交集返回-1
		if(tree[u].l > qr || tree[u].r < ql) return -1;

		//完美包含但不满足返回-1
		if(tree[u].l >= ql && tree[u].r <= qr && tree[u].sum == 0) return -1;
	...
	while(q --)
	{
		...
		if(op & 1)
		{
			...
			while(hld.top[u] != hld.top[v])
			{
				...
				int c = seg.find_first(1, l, r);
				if(c != -1) ans = hld.seq[c];
		...
		else
		{
			int u; cin >>u;
			int pos = hld.in[u];
			int c = seg.rangeSum(1, pos, pos);
			if(c == 1) seg.rangeAdd(1, pos, pos, -1);
			else seg.rangeAdd(1, pos, pos, 1);
		}
	}
