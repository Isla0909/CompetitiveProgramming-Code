...
{	
	...
	int n, m; cin >>n >>m;
	int len = min(n, m);
	vector<vector<array<Z, 2>>> f(n + 1, vector<array<Z, 2>>(len + 1));
	f[0][0][1] = 1;
	for(int i = 1; i <= n; i ++)
		for(int j = 0; j <= min(i - 1, len); j ++)
		{
			Z in = j, out = m - j;
			f[i][j][1] += (f[i - 1][j][1] + f[i - 1][j][0]) * in;
			f[i][j][0] += f[i - 1][j][0] * out;

			if(j + 1 <= len) f[i][j + 1][0] += f[i - 1][j][1] * out;
		}
	Z ans = 0;
	for(int j = 0; j <= len; j ++)
		ans += f[n][j][1];
