...
struct SegmentTree
{
	...
	void rangeSet(int u, int l, int r, i64 x)
	{
	...
	int find_first(int u, int ql, int qr, i64 x)
	{
		//无交集返回-1
	...
	int find_last(int u, int ql, int qr, i64 x)
	{
		//无交集返回-1
		if(tree[u].l > qr || tree[u].r < ql) return -1;
...
signed main()
{
	...
	while(T --)
	{
		...
		while(q --)
		{
			...
			i64 h = a[x];
			if(seg1.max_query(1, x, x) < h)
			{
				int r = seg1.find_first(1, x, n, h);
				if(r == -1) r = n + 1;
				seg1.rangeSet(1, x, r - 1, h);
			}

			if(seg2.max_query(1, x, n) < h)
			{
				int l = seg2.find_last(1, 1, x, h);
				if(l == -1) l = 0;
				seg2.rangeSet(1, l + 1, x, h);
			}

			i64 M = seg1.max_query(1, 1, n);
			int p = seg1.find_first(1, 1, n, M);

			i64 ans = seg1.rangeSum(1, 1, p) - sum;
			if(p < n) ans += seg2.rangeSum(1, p + 1, n);
			cout <<ans <<endl;
		}
	}
	return 0;
