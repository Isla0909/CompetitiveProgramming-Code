...
signed main()
{
	...
	while(q --)
	{
		...
		if(op == 1)
		{
			...
			seg.rangeAdd(1, l, l, c);
			if(r + 1 <= n) seg.rangeAdd(1, r + 1, r + 1, -c);
			if(l + 1 <= n) seg.rangeAdd(1, l + 1, r, d);
			if(r + 1 <= n) seg.rangeAdd(1, r + 1, r + 1, (l - r) * d);
		}
		else
		{
			int x; cin >>x;
			cout <<seg.rangeSum(1, 1, x) <<endl;
		}
	}
	return 0;
}