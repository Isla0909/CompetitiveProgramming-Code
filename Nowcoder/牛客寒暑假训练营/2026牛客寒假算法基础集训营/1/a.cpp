...
signed main()
{
	...
	while(T --)
	{
		...
		vector<Z> a(10);
		a[0] = p[1] * p[2] * p[3] * p[5] * p[6] * p[7] * (1 - p[4]);
		a[1] = p[3] * p[6] * (1 - p[1]) * (1 - p[2]) * (1 - p[4]) * (1 - p[5]) * (1 - p[7]);
		a[2] = p[1] * p[3] * p[4] * p[5] * p[7] * (1 - p[2]) * (1 - p[6]);
		a[3] = p[1] * p[3] * p[4] * p[6] * p[7] * (1 - p[2]) * (1 - p[5]);
		a[4] = p[2] * p[3] * p[4] * p[6] * (1 - p[1]) * (1 - p[5]) * (1 - p[7]);
		a[5] = p[1] * p[2] * p[4] * p[6] * p[7] * (1 - p[3]) * (1 - p[5]);
		a[6] = p[1] * p[2] * p[4] * p[5] * p[6] * p[7] * (1 - p[3]);
		a[7] = p[1] * p[3] * p[6] * (1 - p[2]) * (1 - p[4]) * (1 - p[5]) * (1 - p[7]);
		a[8] = p[1] * p[2] * p[3] * p[4] * p[5] * p[6] * p[7];
		a[9] = p[1] * p[2] * p[3] * p[4] * p[6] * p[7] * (1 - p[5]);
